<#
.SYNOPSIS
    Calculate code metrics for Bucket project

.DESCRIPTION
    This script analyzes the Bucket project source code and provides metrics
...
.NOTES
    Name:        Get-BucketCodeMetrics.ps1
...
$sourceDir = Join-Path $projectRoot "src\Bucket"
...
try {
    ...
    Write-BucketLog "Starting line count for $filesCount files..." -Type Stage
}
catch {
    Write-BucketLog "Error getting files: $($_.Exception.Message)" -Type Error
    exit 1
}
...
foreach ($file in $files) {
    ...
    try {
        ...
        Write-BucketLog "Error processing file $($file.FullName): $($_.Exception.Message)" -Type Error
    }
}
...
Write-BucketLog "Code Metrics Summary" -Type Stage

# Display formatted results header
$summaryBorder = "=" * 60
Write-Host $summaryBorder -ForegroundColor Cyan
Write-Host "  Bucket Code Analysis Results" -ForegroundColor White
Write-Host $summaryBorder -ForegroundColor Cyan
...
Write-BucketLog "Script completed successfully!" -Type Success
Write-BucketLog "Execution time: $formattedTime" -Type Success